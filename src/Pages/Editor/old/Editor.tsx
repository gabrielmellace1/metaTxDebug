...
const EditorParent: React.FC = () => {
  ...
  const { tokenIds, stateSelected } = location.state;
  ...
  const pictureWidth = (maxX - minX + 1) * squareSize;
  const pictureHeight = (maxY - minY + 1) * squareSize;
    ...
    <Flex direction="column" align="center" m={4} p={5} bg="gray.900" color="white">
      <Flex direction="row" w="full">
        <Box flex={{ base: 1, md: "none" }} maxWidth={{ base: "100%", md: "300px" }} bg="gray.700" p={4} borderRadius="md" boxShadow="xl">
          <EditorPicture setPreviewUrl={setPreviewUrl} width={pictureWidth} height={pictureHeight} ownedSquares={editorSquares} />
        </Box>
