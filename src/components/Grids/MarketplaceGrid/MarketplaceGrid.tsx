...
const atlasLayer: Layer = (x, y) => {
  ...
  if (atlas && id in atlas) {
    ...
    return { color: (x + y) % 2 === 0 ? COLOR_BY_TYPE[8] : COLOR_BY_TYPE[9] };
  }
};

const selectedStrokeLayer: Layer = (x, y) => isSelected(x, y) ? { color: '#ff0044', scale: 1.4 } : null;
const selectedFillLayer: Layer = (x, y) => isSelected(x, y) ? { color: '#ff9990', scale: 1.2 } : null;
...
async function handleClick(x: number, y: number) {
  ...
  if (tile) {
    ...
    if (!tile.forSale && tile.owner !== userWallet) {
      return;
    }
      if (tile.isOnState) {
        selected = [{ x, y }];
  ...
  let canBuy = true, canSell = true, canCancel = true;
